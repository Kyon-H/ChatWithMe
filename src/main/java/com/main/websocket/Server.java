...
public class Server extends WebSocketServer {
    ...
    public void onOpen(WebSocket webSocket, ClientHandshake clientHandshake) {
        System.out.println("Server.onOpen()" + webSocket);
        //获取参数中当前用户id
        String param = clientHandshake.getResourceDescriptor();
        userId = param.substring(1, param.length());
//            sendMessage(webSocket, "WebSocket连接 userid:" + userId + " 建立成功");
    ...
    public void onMessage(WebSocket webSocket, String jsonMessage) {
        System.out.println("Server.onMessage()" + jsonMessage);
    ...
    public String getMessage(Message message) {
        System.out.println("Server.getMessage()" + message);
